...
import requests, json, vk_api, os, logging
...
def handle_link(message):
    """Handle link attachments in messages"""
    try:
        # Create a unique identifier for this message+attachment combination
...
def handler(message, skip_reply=False):
    """Main handler to process all message attachments
    
    Args:
        message: The message to process
        skip_reply: If True, skips processing forwarded messages to avoid duplicates
    """
    try:
        ...
        # Only process reply messages if not explicitly skipped
        if not skip_reply:
        ...
        if "fwd_messages" in message and message["fwd_messages"]:
            for fwd_message in message["fwd_messages"]:
                try:
                    ...
                    # Send the forwarded message with appropriate format
                    response = requests.post(
                        f"https://api.telegram.org/bot{telegram_bot_token}/sendMessage",
                        data={
                            "chat_id": telegram_chat_id,
                    ...
                    # Process all attachments in the forwarded message but skip reply handling
                    # to avoid processing nested forwards twice
                    handler(fwd_message, skip_reply=True)
                    
                    # Process nested forwards explicitly
...
# Function to clear processed messages cache (useful for long-running instances)
def clear_processed_messages_cache():
    """Clear the cache of processed message IDs periodically to prevent memory issues"""
