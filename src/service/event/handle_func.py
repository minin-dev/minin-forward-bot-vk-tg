...
import requests, json, vk_api, logging, os
...
def handle_photo(message):
    ...
    for attachment in attachments:
        if attachment.get('type') == 'photo':
            ...
            url = photo.get('orig_photo', {}).get('url') or photo.get('sizes', [{}])[-1].get('url')
            if url:
                image = {"type": "photo", "media": url}
                media_group.append(image)
    if media_group:
        media_group[0]["caption"] = f"<code>{get_user_name(message)} ✉</code>\n\n<pre>{message.get('text', '')}</pre>"
...
def handle_audio_message(message):
    ...
    for attachment in attachments:
        if attachment.get('type') == 'audio_message':
            ...
            audio_url = audio_message.get('link_mp3')
            if not audio_url:
                continue
            audio_data = requests.get(audio_url).content
            response = requests.post(
                f"https://api.telegram.org/bot{telegram_bot_token}/sendVoice",
                files={"voice": ("voice.mp3", audio_data)},
...
def handle_audio(message):
    ...
    for attachment in attachments:
        if attachment.get('type') == 'audio':
            audio_url = attachment['audio'].get('url')
            if not audio_url:
                continue
            audio_file = requests.get(audio_url)
            os.makedirs('other_files', exist_ok=True)
            audio_path = 'other_files/audio.mp3'
            ...
            with open(audio_path, 'rb') as audio_file:
                ...
                data = {
                    ...
                    'performer': attachment['audio'].get('artist', '')
                }
...
def handler(message, skip_reply=False):
    try:
        ...
        if not skip_reply and message.get("fwd_messages"):
        ...
        if message.get("fwd_messages"):
            for fwd_message in message["fwd_messages"]:
                try:
                    ...
                    if fwd_message.get("fwd_messages"):
